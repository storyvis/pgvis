...
```mermaid
flowchart TD
    SC[(SchemaCache)] --> RB[pgvis-rest::build_app]
    SC --> MB[pgvis-mcp::build_mcp_tools]
    SC --> OA[pgvis-rest::openapi::generate_spec]
    CFG[Config] --> RB
    CFG --> MB
    CFG --> OA
...
Crate [pgvis-rest](../crates/pgvis-rest). Entry point
`build_app(cache, config, dialect) -> axum::Router`
...
`AppState` ([routing.rs](../crates/pgvis-rest/src/routing.rs)) holds
`Arc<ArcSwap<SchemaCache>>`, `Arc<Config>`, and `Arc<Dialect>`. The
...
into `RequestBody::Single`/`Bulk`. It then calls `plan_request`. **TODO seam:**
SQL build + execute is not wired in the handler yet, so on success it returns a
JSON plan summary (`{"status":"planned","plan_type":...}`); errors are already
mapped to `err.http_status()` with the `PGRST*` code body — see
[06-errors-and-config.md](06-errors-and-config.md) and
[08-future-scope.md](08-future-scope.md). Known gaps tracked there:
`and=`/`or=` query parsing, relation ordering, and content negotiation.

## OpenAPI `[In progress]`

Module [pgvis-rest/src/openapi.rs](../crates/pgvis-rest/src/openapi.rs). Entry
...
`plan_request`. Same **TODO seam** as REST: it returns a plan summary until the
execute boundary lands. Known parser gaps (select string, order, logic) are
shared with REST and tracked in [08-future-scope.md](08-future-scope.md).

## Delivery wrappers

### `pgvis-embed` `[In progress]`

[pgvis-embed](../crates/pgvis-embed/src/lib.rs) is a fluent `Builder` for host
Rust apps: `Builder::new(dsn).schemas([...]).build().await -> axum::Router`. It
constructs a `PgBackend`; wiring introspection + `build_app` into the returned
router is the remaining step (currently returns an empty router).

### `pgvis-server` `[In progress]`
...
`serve` (default), `openapi`, and `inspect`, and initializes JSON tracing. The
subcommands currently print scaffolding notices; full wiring is
[08-future-scope.md](08-future-scope.md).
